...
interface GenericTaxonomyPageClientProps {
  kind: TaxKind;
...
export function GenericTaxonomyPageClient({
  kind,
  initialNodes,
  initialTree,
  totalCount,
  ...
  const fetchPage = useCallback(
    async (
      ...
      const activeFilters = { ...(overrideFilters ?? filters), kind };
    ...
    [filters, sortField, sortOrder, kind]
  );

  const handleFiltersChange = useCallback(
    (newFilters: TaxonomyFilters) => {
      const merged = { ...newFilters, kind };
      setFilters(merged);
      setCursor(undefined);
      setCursorHistory([]);
